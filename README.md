...
자막과 제목 카드에 등장·퇴장 효과를 준다.

```json
{
  "base": "ttokttok",
  "motion": {
    "subtitle":  { "type": "slide-up", "inMs": 200, "outMs": 120, "distance": 40 },
    "titleCard": { "type": "pop", "inMs": 200, "outMs": 200, "scale": 1.12 }
  }
}
```
...
| `fade` (기본) | `\fad(in,out)` |
| `pop` | 살짝 크게 나타나 제자리로. `scale`로 조절 |
| `slide-up` | 아래에서 올라오며 페이드. `distance`로 조절, 생략하면 글자 크기의 60% |

**`slide-up`의 좌표는 정렬에서 계산된다.** 자막을 상단 정렬로 바꾸면 앵커도 위로 따라가므로 애니메이션이 깨지지 않는다.

**페이드가 큐보다 길면 한 변당 큐의 40%로 잘린다.** 0.8초짜리 큐에 400ms 페이드를 걸면 320ms로 줄어든다. 안 그러면 자막이 제대로 보이기도 전에 사라지거나 깜빡인다.

넣지 않은 것은 **타자기와 카라오케**다. 둘 다 음절 단위 타이밍이 필요한데 이 스킬의 큐는 문장 단위라 그 정보가 없다. 없는 정보로 흉내내면 글자가 엉뚱한 데서 끊긴다.
